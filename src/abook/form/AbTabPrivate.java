...
import abook.form.table.AbPrivateTable;
import abook.form.table.model.AbPrivateTableModel;

/**
 * 秘密タブ
 */
public class AbTabPrivate extends JPanel {

	/** テーブル */
	private AbPrivateTable table;

	/** テーブルモデル */
	private AbPrivateTableModel model;
	 ...
	 */
	public AbTabPrivate(List<AbExpense> expenses) {
		super();

		isReflesh = true;

		// テーブル設定
		model = new AbPrivateTableModel(expenses);
		table = new AbPrivateTable(model);
